...
function buildPrompt(productLink: string, productName: string, description: string) {
...
export async function analyzeProduct(productLink: string, productName: string, description: string): Promise<GeminiResult> {
  ...
  try {
    ...
    if (data?.candidates && data.candidates[0] && data.candidates[0].content) {
      ...
      if (Array.isArray(content)) {
        const piece = content.find((c: any) => c?.text)?.text;
    ...
    if (!text) throw new GeminiError('No text returned from Gemini', 'No text returned from AI service');
